...
원칙 4는 NAS → G드라이브 배포를 요구한다. 유통 인벤토리는 `id + title`이고 사이트 URL이
`<host>/webtoon/<id>`이므로, 이 데이터셋은 **해석 가능한 침해물 색인**이기도 하다. 로컬
보관과 클라우드 배포는 성격이 다른 행위이므로 3단계 진입 여부는 사람이 정한다. 결정 전까지
`stage_to_nas.sh`는 `newtoki_market`을 스테이징하지 않는다(`tests/ci/test_staging_coverage.py`
의 `EXCLUDED`). 이미 병행 세션이 NAS에 올려둔 사본은 별도 판단 대상.
